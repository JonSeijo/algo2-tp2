...
$ $\newline
  ...
  \AbsFc[estr]{diccString($\alpha$)}[e]{d : dicc(string, $\alpha$) / ($\forall s$: string)(c $\in$ claves($d$)) $\impluego$ $e$.Definido(s) $=$ $true$ $\land$ obtener($s$, $d$) $=$ $e$.Obtener($s$) }
...
%crear diccTrie
\begin{algorithm}[H]
{\textbf{Vacio}() $\to$ $res$ : diccString($\alpha$)}
\begin{algorithmic}[1]
...
%CLAVES
\begin{algorithm}[H]
{\textbf{Claves}(\In{d}{diccString($\alpha$)}}) $\to$ $res$ : conj($\alpha$)
\begin{algorithmic}[1]
...
%OBTENER
\begin{algorithm}[H]
{\textbf{Obtener}(\In{p}{string}, \In{d}{diccString($\alpha$)}}) $\to$ $res$ : $\alpha$
\begin{algorithmic}[1]
...
%DEFINIR
\begin{algorithm}[H]
{\textbf{Definir}(\Inout{d}{diccString($\alpha$)}, \In{p}{string}, \In{a}{$\alpha$})}
\begin{algorithmic}[1]
...
%CLAVES
\begin{algorithm}[H]
{\textbf{Borrar}(\In{p}{string}, \Inout{d}{diccString($\alpha$)}})
\begin{algorithmic}[1]
...
\begin{algorithm}[H]
{\textbf{cuentaHijos}(\In{desde}{puntero(nodo)}} $\to$ $res$ : $nat$)
\begin{algorithmic}[1]
