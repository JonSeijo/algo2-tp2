...
Estoy en esto -> Jonathan

$ $\newline

( ) El indice de la posicion del vector es igual al id del juegador en ese indice (de esto se desprende que los ids son unicos) $ $\newline

($\forall i$: nat)((i $<$ Longitud(j.jugadores)) $\impluego$ j.jugadores[$i$].id = i )
$ $\newline


( ) Todo jugador de j.jugadores que este conectado, tiene una posicion que es una coordenada existente en el mapa $ $\newline

($\forall e$: jugador) ((esJugadorConectado($e$)) $\impluego$ PosExistente(j.jugadores[$e$].pos, j.mapa))
$ $\newline


( ) Dimensiones de la grillaJugs (vector de vectores) es igual al tama\~no del mapa $ $\newline
...
( ) No hay elementos repetidos en las listas de grillaJugs  $ $\newline

($\forall x, y$: nat) ((enRango($x$, $y$, $j$.mapa) sinRepetidos($j$.grillaJugs[$x$][$y$]))
$ $\newline

( ) Todo jugador que esta conectado tiene su id en la lista que se encuentra en grillaJugs para su posicion  $ $\newline
...
( ) Toda id en toda lista de grillaJugs es un de un jugador del juego que este conectado  $ $\newline
...
( ) iteradorAPos apunta al elemento correcto (misma id) en la lista de grillaJugs correspondiente a su pos $ $\newline

($\forall e:$ jugador) ( esJugadorConectado($e$, $j$) $\impluego$ Siguiente($j$.jugadores[$e$].iteradorAPos) = $e$ )

$ $\newline

( ) cantCapt es consistente con las cantidades de su lista de pokemons capturados $ $\newline

($\forall e:$ jugador) (esJugadorNoEliminado($e$, $j$) $\impluego$ $j$.jugadores[$e$].cantCapt = sumaCantidades($j$.jugadores[$e$].pokemons))


( ) cantPokemonTotales es igual a la sumatoria de todos los significados del diccionario $ $\newline
...
( ) Para todo pokemon del diccionario, la cantidad que hay es igual a la suma de los salvajes mas los capturados por jugadores (no eliminados) $ $\newline

( ) Para todo pokemon salvaje, su cantidad es la resta entre la cantidad del trie menos los capturados por jugadores (no eliminados) $ $\newline

( ) Dimensiones de pokenodos (vector de vectores) es igual al tama\~no del mapa $ $\newline

( ) Todo pokenodo que tenga un pokestruc, esta en una coordenada valida del mapa $ $\newline

( ) No hay pokenodos con pokestrucs que esten a distancia menor a 5 $ $\newline

( ) El contador de todo pokenodo es $<$ 10 $ $\newline

( ) Todo pokestruc tiene un pokemon que esta bien definido $ $\newline

--- relacionados con heaps, pensar mejor

( ) Todos los ids de la cola de prioridad del pokestruc pertenecen a jugadores conectados que esten en el rango2 con un camino hacia el pokemon, y la cantidad de pokemones del heap y el jugador se corresponen $ $\newline

( ) Si el jugador esta conectado, en rango2 con un camino hacia un pokestruc, entonces esta su id en la cola de prioridad del pokestruc y la cantidad de pokemons se corresponde $ $\newline

( ) Si el jugador esta conectado, en rango2 con un camino hacia un pokenodo, el iterador a la cola de prioridad se corresponde consigo mismo $ $\newline
...
\State Definir($j$.cantPokemon, $p$) \Comment $O(|P|)$
...
\While{$i$ < $Longitud(posiblesCapturadores)$} \Comment $O(EC*log(EC))$
	\State aInsertar $\gets$ <posiblesCapturadores[$i$], $j$.jugadores[posiblesCapturadores[$i$]].cantCapt> \Comment $O(1)$
	\State it $\gets$ Encolar($h$, $aInsertar$) \Comment $O(log(EC))$
