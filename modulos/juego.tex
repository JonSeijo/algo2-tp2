...
\If{$\neg$MovValido($e$, $c$, $j$)}
	...
	\If{$j$.jugadores[$e$].sanciones $\geq$ 5}
		...
		\While{HaySiguiente($pokesJug$)}   \Comment El jugador pudo haber capturado PC pokemons $O(PC*|P|)$
			% Asumo que tengo un iterador para el trie, cuyo siguiente es tupla <poke, cant>
			\State pokemon $pokeActual \gets$ Siguiente($pokesJug$).poke 	\Comment $O(1)$
			\State nat $cantActual \gets$ Siguiente($pokesJug$).cant 	\Comment $O(1)$
			
			\State Definir($j$.cantPokemons, $pokeActual$, Obtener($pokeActual$, $j$.cantPokemon)$-cant$) \Comment $O(|P|)$
		\EndWhile
