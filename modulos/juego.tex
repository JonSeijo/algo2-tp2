...
{$res$ $\igobs$ puedoAgregarPokemon($c$, $j$)}  % Ver que mucho no me convence
[$O(1)$] 
...
{$res$ $\igobs$ hayPokemonCercano($c$, $j$)}  % Ver que mucho no me convence
[$O(1)$] 
...
{$res$ $\igobs$ posPokemonCercano($c$, $j$)}  % Ver que mucho no me convence
[$O(1)$] 
...
{$res$ $\igobs$ entrenadoresPosibles($c$, $es$, $j$)}  % Ver que mucho no me convence
[$O$($cardinal(es)$ * $EC$)] 
...
{$res$ $\igobs$ indiceRareza($p$, $j$)}  % Ver que mucho no me convence
...
{$res$ $\igobs$ cantPokemonsTotales($p$, $j$)}  % Ver que mucho no me convence
...
[$p$ $\in$ todosLosPokemons($j$)] 
{$res$ $\igobs $cantMismaEspecie($p$, $j$)}  % Ver que mucho no me convence
...
\end{Estructura}
    ...
    \begin{Tupla}[pokeStruc]
      \tupItem{poke}{pokemon}
      \tupItem{\\ contador}{nat}%
      \tupItem{\\ entrenadores}{colaEntr(jugYCantCapt)} % Deberia ser como sea que se llame el modulo, cola_de_prioridadJug o pokeHeap, nose, pero en el fondo seria un heap
    \end{Tupla}
    ...
    \begin{Tupla}[jugStruc]
      ...
      \tupItem{\\ iteradorAEntrenadores}{itcolaEntrenador(jugYCantCapt)} % Iterador al heap
...
\State itcolaEntrenador(jugYCantCapt) $itEntrenadores$  \Comment $O(1)$
...
\While{$i$ <\hspace{0.5mm} Longitud($posiblesCapturadores$)} \Comment $O(EC*log(EC))$
  ...
  \State itcolaEntrenador(jugYCantCapt) it $\gets$ Encolar($h$, $aInsertar$) \Comment $O(log(EC) + copiar(tupla <nat, nat>))$
...
\begin{algorithm}[H]
{\textbf{iEntrenadoresPosibles}(\In{c}{coor}, \In{es}{conj(jugador)}, \In{j}{juego}) $\to$ $res$ : conj(jugador)}
\begin{algorithmic}[1]
...
\While{HaySiguiente($itPosibles$)}   \Comment O($cardinal(es)*EC$)

    \For{$i \gets$ 0 $\to$ Longitud($jugsEnNodo$) - 1} \Comment $O(EC)$
        \If{Siguiente($itPosibles$) = $jugsEnNodo$[$i$]}   \Comment $O(1)$
...
\For{$i \gets$ 0 to Longitud($j$.jugadores) $-$ 1}   \Comment $O(J)$
...
\begin{algorithm}[H]
{\textbf{iSumarUnoEnJug}(\In{p}{pokemon}, \In{e}{jugador},  \Inout{j}{juego})}
\begin{algorithmic}[1]
...
% Lo defino bien
\If{Def?($p$, $j$.jugadores[$e$].pokemons)} \Comment $O(|P|)$
...
% < de la tupla
\begin{algorithm}[H]
{\textbf{<}(\In{jc1}{jugYCantCapt}, \In{jc2}{jugYCantCapt})  $\to$ $res$ : bool}
\begin{algorithmic}[1]

\If{$jc1$.cant = $jc2$.cant}    \Comment $O(1)$
