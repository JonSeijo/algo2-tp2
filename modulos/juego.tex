...
% crearJuego
Tendriamos que ver como decimo que el mapa lo guardamos por referencia
\begin{algorithm}[H]
...
$ $\newline
\For{$i \gets 0$ to Tam($map$) $-$ 1}
	
	\State vector(lista(jugador)) $vectorInterno \gets$ Vacio()
	
	\For{$j \gets 0$ to Tam($map$) $-$ 1}
		\State lista(jugador) $jugsVacia \gets$ Vacia()
		\State AgregarAtras($vectorInterno$, $jugsVacia$)	
	\EndFor
	
	AgregarAtras($jugs$, $vectorInterno$)
...
\For{$i \gets 0$ to Tam($map$) $-$ 1}		
	\State vector(puntero(pokeStruc)) $vectorInterno \gets$ Vacio()
	\For{$j \gets 0$ to Tam($map$) $-$ 1}
		\State puntero(pokeStruc) $pokePuntero \gets$ NULL
		\State AgregarAtras($vectorInterno$, $pokePuntero$)	
	\EndFor
	AgregarAtras($pokenodos$, $vectorInterno$)
\EndFor
...
\medskip
\Statex \underline{Complejidad:} $O(1)$
\Statex \underline{Justificaion:} Crear vectores vacios cuesta O(1), y el mapa lo pasamos por referencia.
...
\State nat $proxId$ $\gets$ Longitud($j$.jugadores) 
...
\State coor $pos$ $\gets$ CrearCoor(0,0) 

\State vector($<$pokemon, nat$>$) $pokes$ $\gets$ Vacio()

% Le pongo 0 pero en realidad no nos importa porque no hay que usarlo si no tiene pokemon cerca
\State nat $indiceEnPokenodo$ $\gets$ 0 

% Creo el iterador de una lista vacia para poder crearlo
\State lista(nat) $listaDummy$ $\gets$ Vacia()
\State itLista(nat) $iteradorAPos$ $\gets$ CrearIt($listaDummy$)

\State $j$.jugadores.AgregarAtras($<proxId$, 0, $false$, $pos$, $pokes$, $indiceEnPokenodo$, $iteradorAPos>$)

\State $res \gets proxId$

\medskip
\State \underline{Complejidad:} $O(1)$
\State \underline{Justificaion:} Todo cte
