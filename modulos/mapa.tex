...
\If{latitud($c$) $>=$ tam($m$) $\lor$ longitud($c$) $>=$ tam($m$)} \Comment $O(1)$
...
\medskip
\Statex \underline{Complejidad:} $O(1)$
\Statex \underline{Justificacion:} Evaluar la guarda del if es O(1) por que son comparaciones de nats. Si es verdadera se produce una asigacion O(1). Si es falsa se ejecutan asignaciones O(1) y acceso directo a vector O(1). O(1) + O(1) + O(1) + O(1) = O(1). Es constante en cualquier caso.
...
\State $res \gets m.grilla[Latitud(c1)][Longitud(c1)][Latitud(c2)][Longitud(c1)]$ \Comment $O(1)$
...
\While{$\neg EsVacia(aRecorrer)$}    \Comment Como maximo se recorren todas las coordenadas del mapa // $O((m.tam)^2)$
