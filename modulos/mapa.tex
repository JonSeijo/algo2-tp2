...
\For{$i = 0$ to $m.tam$}	    \Comment Fors anidados. Cada uno m.tam iteraciones // $O((m.tam)^2)$
	\State vector(coor) $visitadosAux \gets$ Vacio()  \Comment $O(1)$	
	\For{$j = 0$ to $m.tam$}      \Comment Realiza m.tam iteraciones $O(m.tam)$
...
\State coor $act \gets$ Proximo($aRecorrer$) \Comment Obtener el proximo de la cola es O(1) // $O(1)$
