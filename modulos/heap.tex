\begin{Interfaz}
  ...
  \textbf{g\'eneros}: \TipoVariable{colaEntr}, \TipoVariable{itcola(entrenador)}.
  ...
  \InterfazFuncion{Encolar}{\Inout{h}{colaEntr}, \In{j}{entrenador}}{itcola(entrenador)}
  [$h \igobs h_{0}$]
  ...
  \InterfazFuncion{Proximo}{\In{h}{colaEntr}}{itcola(entrenador)}
  [$\neg$ vacia?(h)]  
  ...
  \InterfazFuncion{Borrar}{\In {it}{itcola(entrenador)}, \In{h}{colaEntr}}{}%
...
\begin{Representacion}
  ...
  \begin{Estructura}{itcola(entrenador)}[estr]
      \begin{Tupla}[estr]%
...
[H]{\textbf{iEncolarElem}(\Inout{h}{colaEntr}, \In{j}{entrenador}) $\to$ $res$ : itcola(entrenador)}
    	\begin{algorithmic}[1]
    	\State $Nodoheap$ n $\gets <j, NULL, NULL, NULL>$ \Comment $O(1)$
    	\State $puntero(Nodoheap)$ p $\gets n$  \Comment $O(1)$
		\If{$EsVacia?(h)$} \Comment Insertamos el nodo al final del heap $O(1)$
...
[H]{\textbf{iBorrar}(\In{it}{itcola(entrenador)}, \In{h}{}colaEntr)}
    	\begin{algorithmic}[1]
    		...
    		\Else    		
				...
				\If{($it.estructura \rightarrow ultimo \rightarrow padre) \not= aPonerUltimo$} \Comment Swapeamos con el \'ultimo del heap. $O(1)$	
...
\begin{algorithm}[H]{\textbf{iCrearItCola}(\In{h}{colaEntr}, \In{p}{puntero(Nodoheap)}) $\to$ $res$ : itcola(entrenador)} $ $\newline
\Comment Funci\'on privada que devuelve un iterador a un elemento de la cola de prioridad.
    	\begin{algorithmic}[1]
			...
			\Statex \underline{Complejidad:} $O(1)$
			\Statex \underline{Justificaci\'on:} Todas las operaciones son $O(1)$
			Pre: si el puntero es $\not=$ NULL, tiene que apuntar a un nodo perteneciente a la estructura de la cola de entrenadores. Si es NULL, entonces la cola de entrenadores tiene que ser vac\'ia. Post: se devuelve un iterador a dicho nodo, o en caso de que sea vac\'ia devuelve un iterador que no apunta a ning\'un nodo
    	\end{algorithmic}
\end{algorithm}
