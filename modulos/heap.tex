...
\begin{algorithm}[H]{\textbf{EncolarElem}(\Inout{h}{heap(tupla$<$id: nat, cantCapt: nat$>$)}, \In{j}{tupla$<$id: nat, cantCapt: nat$>$}) $\to$ $res$ : itHeap(tupla$<$id: nat, cantCapt: nat$>$)}
		...
		\If{$h.raiz \not= h.ultimo$} \Comment $O(1)$
			...
			\While{$(nuevoNodo \rightarrow padre) \not=$ NULL}
				...
				\EndIf			
				\State $nuevoNodo \gets (nuevoNodo \rightarrow padre)$ \Comment $O(1)$
			\EndWhile
		\EndIf
...
{\textbf{Swap}(\Inout{h}{heap(tupla$<$id: nat, cantCapt: nat$>$), \In{nuevoNodo}{puntero(Nodoheap)}})} \Comment Esta funci\'on es privada y swapea dos nodos cambiando los punteros.
    	\begin{algorithmic}[1]
