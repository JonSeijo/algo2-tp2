...
\begin{algorithmic}[1]
    ...
    \Else
        ...
        \EndIf
    \EndIf
    \State nodo m $\gets$ *($c$.ultimo)
    \State SiftUp($m$, $c$) \Comment $O(log(\#(c)))$
    \State puntero(nodo) $p$ $\gets$ $\&m$
    \State res $\gets$ crearItColaEntr($p$, $c$) \Comment $O(1)$

    NO OLVIDAR DEVOLVER ITERADOR AL ULTIMO, completar la \'ultima l\'inea
...
\textbf{Pre} $\equiv$ \{El nodo esta en la estructura de C\}\\%
...
\begin{algorithm}[H]
{\textbf{iSiguiente}(\Inout{it}{itColaEntr($\alpha$)}) $\to$ $res$ : $\alpha$}
\begin{algorithmic}[1]
...
\begin{algorithm}[H]
{\textbf{iBorrar}(\In{it}{itColaEntr($\alpha$)}, \Inout{c}{colaPrioridad($\alpha$)})}
\begin{algorithmic}[1]
...
\begin{algorithm}[H]
{\textbf{icrearItColaEntr}(\In{p}{puntero(nodo)}, \In{c}{colaPrioridad($\alpha$)}) $\to$ $res$ : itColaEntr($\alpha$)}
\begin{algorithmic}[1]
